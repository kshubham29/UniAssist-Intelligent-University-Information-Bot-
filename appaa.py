...
# GEMINI SETUP
# ====================================
api_key = "AIzaSyDglFaGGeq2fp6Wt69MDyKLYa0-m1O0xek"
...
#    ImportError = file missing
#    Other errors = missing pip packages inside Voice_module.py
# ====================================
...
try:
    ...
    VOICE_ERROR = f"Voice_module.py not found: {e}"
except Exception as e:
    # This catches PortAudio, pyttsx3, sounddevice etc. crashing at import
    VOICE_ERROR = str(e)
...
.greeting-card::before {
    ...
    background: conic-gradient(from 0deg at 50% 50%,
        transparent 0deg, rgba(197,165,114,0.03) 60deg,
        transparent 120deg, rgba(26,111,255,0.02) 180deg,
        transparent 240deg, rgba(197,165,114,0.03) 300deg, transparent 360deg);
...
.suggestions { display: flex; flex-wrap: wrap; gap: 8px; justify-content: center; position: relative; }
.suggest-chip {
    background: rgba(197,165,114,0.06); border: 1px solid rgba(197,165,114,0.2);
    border-radius: 20px; padding: 7px 16px; font-size: 12.5px; color: var(--gold);
    cursor: pointer; transition: all 0.2s; font-family: 'DM Sans', sans-serif;
}
.suggest-chip:hover {
    background: rgba(197,165,114,0.14); border-color: var(--gold);
    transform: translateY(-2px); box-shadow: 0 4px 16px rgba(197,165,114,0.15);
}

.chat-area { display: flex; flex-direction: column; gap: 4px; padding-bottom: 12px; }

.msg-row-user {
    display: flex; justify-content: flex-end;
    animation: slideRight 0.35s cubic-bezier(0.16,1,0.3,1) both; margin-bottom: 6px;
}
.msg-row-bot {
    display: flex; justify-content: flex-start; align-items: flex-start; gap: 10px;
    animation: slideLeft 0.35s cubic-bezier(0.16,1,0.3,1) both; margin-bottom: 6px;
}
...
.bot-avatar {
    ...
    box-shadow: 0 0 16px rgba(197,165,114,0.25); margin-top: 4px;
}
.bubble-user {
    background: linear-gradient(135deg, #162848, #0c1e38);
    border: 1px solid rgba(26,111,255,0.2);
    border-radius: 20px 20px 4px 20px; padding: 13px 18px;
    max-width: 68%; color: #c8d8f0; font-size: 14.5px; line-height: 1.65;
...
.bubble-bot {
    ...
    max-width: 78%; color: var(--text); font-size: 14.5px; line-height: 1.75;
    box-shadow: 0 4px 20px rgba(0,0,0,0.35); position: relative;
}
.bubble-bot::before {
    content: ''; position: absolute; top: 0; left: 0; right: 0; height: 1px;
    background: linear-gradient(90deg, var(--gold-dim), transparent);
    border-radius: 4px 20px 0 0; opacity: 0.5;
}

.stChatInputContainer { background: transparent !important; }
.stChatInputContainer > div {
    background: rgba(13,24,48,0.95) !important; border: 1px solid var(--border) !important;
...
.sb-section {
    ...
    margin: 16px 0 10px; padding-bottom: 6px; border-bottom: 1px solid var(--border2);
}
...
.voice-orb-wrap { position: relative; width: 80px; height: 80px; margin: 0 auto 12px; }
.voice-orb {
    ...
    font-size: 30px; box-shadow: 0 0 24px rgba(197,165,114,0.15), inset 0 1px 0 rgba(197,165,114,0.1);
    transition: all 0.3s; position: relative; z-index: 2;
}
.voice-ring {
    position: absolute; inset: -8px; border-radius: 50%;
    border: 1px solid rgba(197,165,114,0.15); animation: ringPulse 2.5s ease-in-out infinite;
}
...
.wave-row { display: flex; align-items: center; justify-content: center; gap: 3px; height: 28px; margin: 8px 0; }
.wbar { width: 3px; border-radius: 3px; background: var(--gold); opacity: 0.2; transition: opacity 0.3s; }
...
.voice-status { font-size: 11.5px; color: var(--text2); margin-bottom: 10px; min-height: 16px; }
.voice-last {
    background: rgba(0,0,0,0.3); border: 1px solid var(--border2); border-radius: 8px;
    padding: 7px 10px; font-size: 11.5px; color: var(--text2); font-style: italic;
    text-align: left; min-height: 30px; margin-bottom: 10px; word-break: break-word;
}
...
.stButton > button {
    ...
    transition: all 0.2s !important; width: 100% !important; letter-spacing: 0.3px !important;
}
...
.citation-card {
    background: rgba(13,24,48,0.7); border: 1px solid var(--border2);
    border-left: 3px solid var(--gold-dim); border-radius: 10px;
...
[data-testid="stMetric"] {
    background: rgba(13,24,48,0.7) !important; border: 1px solid var(--border) !important;
...
with st.sidebar:
    ...
    if VOICE_AVAILABLE:
        ...
        if st.session_state.voice_last:
            st.markdown(f'<div class="voice-last">🗣️ "{st.session_state.voice_last}"</div>',
                        unsafe_allow_html=True)

    else:
        # ✅ Now shows the REAL error so you know what to fix
        st.markdown(f"""
        <div style="background:rgba(255,77,109,0.08);border:1px solid rgba(255,77,109,0.2);
            ...
            ❌ Voice unavailable<br>
            <span style="font-size:10px;color:#ff9999;word-break:break-word;">{VOICE_ERROR[:120]}</span>
        </div>
        """, unsafe_allow_html=True)
        st.markdown("**Fix:** Run in your terminal:")
        st.code("pip install SpeechRecognition sounddevice scipy pyttsx3", language="bash")
    ...
    if st.session_state.citations:
        with st.expander("📚 Source References", expanded=False):
            for i, chunk in enumerate(st.session_state.citations):
                st.markdown(
                    f'<div class="citation-card"><strong style="color:var(--gold);">Source {i+1}</strong>'
                    f'<br>{chunk[:350]}...</div>',
                    unsafe_allow_html=True)

    st.markdown('<div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--border2);">',
                unsafe_allow_html=True)
    cols = st.columns(3)
